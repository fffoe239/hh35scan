...
def check_public_sources(value):
    ...
    for source in CHECK_SOURCES:
        ...
        try:
            ...
            results.append({
                "name": source["name"],
                "status": "مستخدم" if found else "لم يظهر",
                "detail": "ظهر في نتائج الموقع" if found else "لا توجد إشارة واضحة في النتائج العامة",
                "url": url,
            })
        except requests.RequestException:
            results.append({
                "name": source["name"],
                "status": "غير قابل للتأكيد",
                "detail": "تعذر الوصول إلى المصدر",
                "url": url,
            })
    return results
        ...
        if kind == "invite":
            ...
            try:
                ...
                if response.status_code == 200:
                    results.append({"name": "Discord", "status": "مستخدم", "detail": "الاختصار موجود في Discord", "url": invite_url})
                elif response.status_code in (400, 404):
                    results.append({"name": "Discord", "status": "غير موجود", "detail": "لم يتم العثور على الاختصار", "url": invite_url})
                else:
                    results.append({"name": "Discord", "status": "غير قابل للتأكيد", "detail": "استجابة غير واضحة من Discord", "url": invite_url})
            except requests.RequestException:
                results.append({"name": "Discord", "status": "غير قابل للتأكيد", "detail": "تعذر الوصول إلى Discord", "url": invite_url})
        else:
            results = check_public_sources(value)
...
def admin():
    ...
    with db() as connection:
        if role == "owner":
            ...
            rows = connection.execute("SELECT * FROM codes WHERE created_by='friend' ORDER BY id DESC").fetchall()
