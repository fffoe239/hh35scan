...
def admin_required(view):
    ...
    def wrapped(*args, **kwargs):
        return view(*args, **kwargs) if session.get('admin_role') else redirect(url_for('home'))
    return wrapped
...
def generate_names(length, amount=40, shortcut=False):
    ...
    while len(values)<amount:
        ...
        if length >= 3 and random.random() < .35: chars[random.randrange(1,length-1)]='.'
        value=''.join(chars).strip('.')
        if value and '..' not in value: values.add(value)
...
def home():
    if request.method == 'POST':
        ...
        if role:
            session.clear(); session['admin_role']=role; return redirect(url_for('admin'))
